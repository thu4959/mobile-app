...
// 追加: cornersLine を定義（4隅のマス目を格納する配列）
const cornersLine = cornersLineFunc([a_1, a_3, c_1, c_3]);
...
window.addEventListener("DOMContentLoaded", function() {
    ...
    // ページ読み込み時にsessionStorageから難易度レベルを取得
    // デフォルトはLevel1
    let level = sessionStorage.getItem("tic_tac_toe_access");
    if (!level) {
        level = "1";
        sessionStorage.setItem("tic_tac_toe_access", level);
    }
    
    // レベルボタンの表示を設定
    levelSetting(parseInt(level) - 1);
}, false);
...
// cornersLine関数を定義
// ************************************
function cornersLineFunc(squareArray) {
...
function bearTurn() {
    ...
    // 3. 真ん中（b_2）が空いていたら優先的に選ぶ（全レベル共通）
    const centerSquare = document.getElementById(b_2);
    if (centerSquare && !centerSquare.classList.contains("js-pen-checked") && !centerSquare.classList.contains("js-bear-checked")) {
        console.log("Bear taking center!");
        isSelect(centerSquare);
        return;
    }
    
    // 4. Level3の場合、カウンター（counter）が1ターン目で、
    //    真ん中（b_2）が選ばれていたら、4隅（a_1, a_3, c_1, c_3）のうち、
    //    ランダムに1マス選ぶ
    if (level === "3" && counter === 1) {
        const centerSquare = document.getElementById(b_2);
        if (centerSquare && centerSquare.classList.contains("js-pen-checked")) {
            const availableCorners = cornersLine.filter(function(square) {
                return square && !square.classList.contains("js-pen-checked") && !square.classList.contains("js-bear-checked");
            });
            
            if (availableCorners.length > 0) {
                const n = Math.floor(Math.random() * availableCorners.length);
                console.log("Bear taking corner on first turn!");
                isSelect(availableCorners[n]);
                return;
            }
        }
    }
    
    // 5. クリックできるマス目の中から、1つをランダムに選ぶ。
